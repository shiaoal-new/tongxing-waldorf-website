...
- [x] 添加 preload 链接到 `_document.tsx` (待验证，部分版本可能已移除)
- [x] 将 CSS background-image 改为原生 poster 属性
- [x] 实现响应式海报选择逻辑
- [x] 优化海报图片文件大小 (质量 70)
- [x] **新增：使用响应式 `<source>` 标签优化视频加载，解决 Mobile Safari 重复请求问题**
- [x] 创建图片重新优化脚本
- [x] 编写优化文档
...
### 问题描述
在移动端 Safari 上，由于组件状态初始化导致 `video` 标签的 `src` 属性先指向桌面版后再切换到移动版，引起浏览器发出两个请求（桌面版和移动版视频各一个）。

### 解决方案
在 `MediaRenderer.tsx` 中移除 `video` 的 `src` 属性，改用带有 `media` 查询的 `<source>` 标签：
...
### 优势
- **减少带宽消耗**: 浏览器在解析 HTML 时就能根据媒体查询决定下载哪个视频，不会再下载不需要的版本。
- **提升加载速度**: 避免了二次请求导致的延迟。
- **更好的原生支持**: 利用浏览器的原生机制处理响应式资源。

## 🎯 总结

通过海报和视频的双重优化，Hero Section 的加载体验得到了显著提升。特别是解决了 Mobile Safari 的重复请求问题，这对于移动端性能至关重要。
