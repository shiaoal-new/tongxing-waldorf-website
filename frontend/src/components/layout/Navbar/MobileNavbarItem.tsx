...
import { MegaMenuContent } from './MegaMenu/ItemTypes';
...
export function MobileNavbarItem({ item, router, actionHandlers, showBackgroundGrid }: MobileNavbarItemProps) {
    ...
    if (item.children && item.children.length > 0) {
        const isMegaMenu = item.layout === 'megamenu' || (item.featuredItems && item.featuredItems.length > 0);
        return (
            <Disclosure as="li" className="w-full">
                {({ open }) => (
                    <>
                        ...
                        <DisPanel className={isMegaMenu ? "pt-1" : "px-4 pb-2 pt-1"}>
                            {isMegaMenu ? (
                                <div className="bg-brand-bg/50 dark:bg-brand-structural/50 rounded-lg overflow-hidden my-2">
                                    <MegaMenuContent item={item} currentPath={currentPath} isDropdown={false} />
                                </div>
                            ) : (
